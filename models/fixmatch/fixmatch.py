...
class FixMatch:
    ...
    def train(self, args, logger=None, progressbar=None):
        ...
        for (x_lb, y_lb), (x_ulb_w, x_ulb_s, _) in zip(
            ...
            with amp_cm():
                ...
                # print(logits_x_lb.shape, y_lb.shape)
            ...
            with torch.no_grad():
                ...
                train_accuracy = accuracy(logits_x_lb, y_lb, segmentation = self.segmentation)
                train_accuracy = train_accuracy[0]

                metric = MulticlassJaccardIndex(num_classes=self.num_classes, average='micro')
                if self.segmentation:
                    preds = torch.max(logits_x_lb, dim=[-1, 1][self.segmentation])[1]
                    train_mIoU = metric(preds.cpu(), y_lb.cpu())
    ...
    def evaluate(self, eval_loader=None, args=None):
        ...
        for x, y in eval_loader:
            ...
            num_batch = x.shape[0]
            total_num += x.numel() #num_batch
            logits = eval_model(x)

            loss = F.cross_entropy(logits, y.long(), reduction="mean")

            preds = torch.max(logits, dim=[-1, 1][self.segmentation])[1]

            acc = torch.sum(preds == y)

            if self.segmentation:
                mIoU += metric(preds.cpu(), y.cpu())
